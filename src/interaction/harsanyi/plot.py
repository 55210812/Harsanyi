...
def plot_multi_line_chart(data, xlabel, ylabel, title, save_folder, save_name, font=20):
    os.makedirs(save_folder, exist_ok=True)
    plt.figure(figsize=(8, 6))
    plt.title(title, fontsize=font)
    X = data[xlabel]
    for y_label in ylabel:
        plt.plot(X, data[y_label], label=y_label)
    plt.xlabel(xlabel, fontsize=font)
    plt.ylabel("q", fontsize=font)
...
def plot_interaction_progress(interaction, save_path, order_cfg="descending", title="",
                              cmap_name="viridis", hline_color="red", font=20):
    ...
    for i, item in enumerate(interaction):
        ...
        plt.hlines(0, 0, X.shape[0], linestyles="dotted", colors=hline_color)
        ...
        plt.xlabel("patterns (with I(S) descending)", fontsize=font)
        plt.ylabel("I(S)", fontsize=font)
