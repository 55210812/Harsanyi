...
from .modify_bert import ModifiedBert

def load_model(model_path):
    config = BertGenerationConfig.from_pretrained(model_path)
    config.is_decoder = True
    return ModifiedBert.from_pretrained(model_path,config=config)

def load_tokenizer(tokenizer_path):
    return AutoTokenizer.from_pretrained(tokenizer_path)


class CalculatorBert(nn.Module):
    """
    Calculator model for the lamma
    ...
    def forward(self,
        ...
        if input_sentences is not None:
            input_ids = self.tokenizer(input_sentences,return_token_type_ids=False,return_tensors="pt")['input_ids']
