...
def main():
    # 设置命令行参数
    parser = argparse.ArgumentParser(description='Community detection from interaction data using K-means clustering')
    ...
    parser.add_argument('--n_clusters', type=int, default=6, help='Number of clusters (default: 3)')
    ...
    if n == 0:
        ...
        # 执行K-means聚类
        #kmeans = KMeans(n_clusters=6, n_init='auto', max_iter=300, init='k-means++').fit(features)
    ...
    # 打印社区结果
    output_path = f"/mnt/data/hqdeng7/CSCIENCE/interaction_nlp_harsanyi/group/kmeans_img_louvain/dog/{args.output}.txt"
    print("K-means聚类社区检测结果:")
    with open(output_path, 'w') as f:
        f.write("K-means聚类社区检测结果:\n")
    ...
    # if args.output:
    #     output_path = f"/mnt/data/hqdeng7/CSCIENCE/interaction_nlp_harsanyi/group/kmeans_img_louvain/dog/{args.output}"
