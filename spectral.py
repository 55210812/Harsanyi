...
def main():
    # 设置命令行参数
    parser = argparse.ArgumentParser(description='Community detection from interaction data using Spectral Clustering')
    ...
    parser.add_argument('--n_clusters', type=int, default=7, help='Number of clusters (default: 3)')
    parser.add_argument('--affinity', default='nearest_neighbors', 
                       choices=['nearest_neighbors', 'rbf', 'precomputed'],
                       help='Affinity matrix type (default: nearest_neighbors)')
    ...
    if n == 0:
        ...
        # 创建邻接矩阵
        #adj_matrix = nx.to_numpy_array(G, weight='abs_weight')
        adj_matrix = nx.to_numpy_array(G, weight='weight')
        # 动态调整n_neighbors参数
        n_neighbors = min(10, n-1) if n > 1 else 1
        
        # 执行谱聚类
        spectral = SpectralClustering(n_clusters=min(args.n_clusters, n),
    ...
    # 打印社区结果
    output_path = f"/mnt/data/hqdeng7/CSCIENCE/interaction_nlp_harsanyi/group/spectral_img_-_weight/xlcat/{args.output}.txt"
    ...
    # if args.output:
    #     output_path = f"/mnt/data/hqdeng7/CSCIENCE/interaction_nlp_harsanyi/group/spectral_shapley_2/{args.output}"
